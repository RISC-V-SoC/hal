...
static struct SpiDevice_transfer* pushTransferStart(void) {
    ...
    if (circularBuffer.cleanupPending[circularBuffer.pushPointer]) {
        if (circularBuffer.buf[circularBuffer.pushPointer].tx_buf != nullptr) {
            free(circularBuffer.buf[circularBuffer.pushPointer].tx_buf);
            circularBuffer.buf[circularBuffer.pushPointer].tx_buf = nullptr;
        }
        circularBuffer.cleanupPending[circularBuffer.pushPointer] = false;
    }
...
static void popTransferComplete(void) {
    ...
    circularBuffer.operationPending[circularBuffer.popPointer] = false;
